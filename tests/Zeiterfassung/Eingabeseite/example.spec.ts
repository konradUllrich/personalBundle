...
let preparedCases: (Awaited<ReturnType<Eingabeseite["prepareCases"]>>[number] &
  casesType)[] = [];

type casesType = Parameters<Eingabeseite["prepareCases"]>[0][number] & {
...
test.beforeAll(async ({ ix, page }) => {
  await ix.clear();
  eingabeseite = new Eingabeseite(ix, page);
...
cases
  ...
  .forEach((caseItem, index) => {
    ...
    test(testName, async () => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
...
cases
  ...
  .forEach((caseItem, index) => {
    test(`Dienstabbruch: ${caseItem.name}`, async ({}) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
      const testBuilder = new TestBuilder(eingabeseite);
...
cases
  .filter((i) => i.azvTag)
  .forEach((caseItem, index) => {
    test(`AZV Tag ${caseItem.name}`, async ({}) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
      const testBuilder = new TestBuilder(eingabeseite);
...
cases
  ...
  .forEach((caseItem, index) => {
    test(`Saldo = 0 : ${caseItem.name}`, async ({}) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
      const testBuilder = new TestBuilder(eingabeseite);
      await eingabeseite.setAnwesenheit({
...
cases
  ...
  .forEach((caseItem, index) => {
    test(`Saldo = -Soll : ${caseItem.name}`, async ({}) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
      const testBuilder = new TestBuilder(eingabeseite);
      await eingabeseite.setAnwesenheit({
...
cases
  .filter((i) => i.plausi)
  .forEach((caseItem, index) => {
    test(`plausi ${caseItem.name}`, async ({ page }) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
      ...
      await expect(
        page.getByText("Eine oder mehrere").nth(0)
      ).not.toBeInViewport();
...
cases
  .filter((i) => i.plausi)
  .forEach((caseItem, index) => {
    test(`buchungsarten ${caseItem.name}`, async ({ page }) => {
      await preparedCases.find((i) => i.name === caseItem.name).user.login();
      await eingabeseite.gotoEingabeseite();
